(() => {
  ...
  const safe = audit.safe || ((v)=>{ try { const u=new URL(v); return ['http:','https:'].includes(u.protocol)?u.href:'#'; } catch { return '#'; } });
  const themeId = audit.themeId || ((t)=>t?.id || t?.theme_id || '');
  const verifiedEvidence = audit.verifiedEvidence || ((t)=>t?.evidence || []);
  const sourceAnchor = audit.sourceAnchor || ((url,label='Mở nguồn')=>{ const href=safe(url); return href==='#'?'':`<a href="${esc(href)}" target="_blank" rel="noopener noreferrer">${esc(label)}</a>`; });
    ...
    anchor.insertAdjacentHTML('afterend', `
      <div class="section-head overview-section attention-head">
        <div class="overview-section-copy">
          ...
          <p class="muted">Không phải tín hiệu mua hay lời khuyên kinh doanh. Mỗi mục phải truy ngược được tới evidence hoặc được gắn rõ là hypothesis/discovery.</p>
        </div>
      </div>
  ...
  function attentionCard(x){
    return `<article class="panel attention-card">
      ...
      ${x.auditHtml || ''}
      <div class="card-footer muted small">${x.sourceHtml || ''}${x.sourceHtml && x.evidence ? ' · ' : ''}${esc(x.evidence || '')}</div>
    </article>`;
  }
  ...
  function buildQueue({policy,money,regional,convergence,corporate,contradiction}){
    ...
    (convergence?.entities || [])
      .filter(x=>x.status!=='not_observed')
      .sort((a,b)=>(b.convergence_score||0)-(a.convergence_score||0))
      .slice(0,2)
      .forEach(e=>{
        const primary=Number(e.primary_evidence_count||0), media=Number(e.media_evidence_count||0);
        out.push({
          kind:'ENTITY', level:e.status==='watch'?'WATCH':'INVESTIGATE', title:e.label, why:e.why_now,
          question:(e.investigation_questions||[])[0] || 'Driver kinh tế nào đang thực sự thay đổi quanh entity này?',
          evidence:`Primary ${primary} · Media ${media} · ${(e.evidence_families||[]).length} họ bằng chứng`,
          sourceHtml:sourceAnchor((e.evidence||[])[0]?.source_url,'Nguồn đầu tiên'),
          auditHtml:audit.entityDetails ? audit.entityDetails(e) : ''
        });
      });
    ...
    (money?.themes || []).slice().sort((a,b)=>{
      ...
      const id=themeId(t), c=cMap[id], adjusted=c?.tension_adjusted_score ?? t.score;
      ...
      out.push({
        ...
        evidence:`Score gốc ${t.score ?? '—'} → sau phản chứng ${adjusted} · ${verified.length} evidence verified`,
        sourceHtml:sourceAnchor(verified[0]?.source_url,'Nguồn đầu tiên'),
        auditHtml:audit.evidenceDetails ? audit.evidenceDetails(t,c) : ''
      });
    });

    (regional?.regions || []).filter(r=>r.state==='dual_acceleration').slice(0,1).forEach(r=>out.push({
      kind:'REGION', level:'DIVERGENCE', title:r.region, why:r.interpretation,
      ...
      sourceHtml:sourceAnchor(r.evidence?.fdi_source_url || r.evidence?.iip_source_url,'Nguồn số liệu'),
      auditHtml:audit.regionDetails ? audit.regionDetails(r) : ''
    }));

    const corp=(corporate?.recent_events || [])[0];
    if(corp) out.push({
      kind:'CORPORATE ACTION', level:'PRIMARY TRIGGER', title:`${corp.ticker || 'DN'} · ${corp.title}`,
      ...
      evidence:corp.publisher || 'Official disclosure', sourceHtml:sourceAnchor(corp.source_url,'Mở công bố')
    });
    ...
    if(out.length < 5){
      ...
      if(p) out.push({
        ...
        evidence:`Strategic relevance ${p.strategic_relevance}`, sourceHtml:sourceAnchor(p.source_url,'Nguồn policy')
      });
    }
  ...
  function normalizeExecutionTab(){
    const apply=()=>{
      const tab=document.querySelector('.tab[data-tab="edge"]'); if(tab) tab.textContent='Execution Profile';
      const edge=document.getElementById('edge');
      const h=edge?.querySelector('.section-head h2'); if(h) h.textContent='Profile thực thi cho Advanced campaigns';
      const eye=edge?.querySelector('.section-head .eyebrow'); if(eye) eye.textContent='ADVANCED EXECUTION PROFILE';
    };
    apply();
    const nav=document.querySelector('.tabs');
    if(nav){ const obs=new MutationObserver(apply); obs.observe(nav,{childList:true,subtree:true}); setTimeout(()=>obs.disconnect(),5000); }
  }
