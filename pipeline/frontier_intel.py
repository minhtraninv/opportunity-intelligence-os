...
RULES={
'attention_distribution':('monthly active','active users','người dùng','users','traffic','lưu lượng','downloads','creator','kol','influencer','followers','algorithm','thuật toán','feed','recommendation','distribution','phân phối','reach','mạng xã hội','social media','platform','nền tảng','community','threads','tiktok','youtube','instagram','facebook','twitter','reddit','substack','discord'),
'technology_frontier':('artificial intelligence','trí tuệ nhân tạo',' ai ','agent','foundation model','large language model','llm','robot','robotics','humanoid','chip','semiconductor','bán dẫn','quantum','autonomous','drone','satellite','biotech','gene editing','battery','solid-state','computer use','multimodal','open source model','open-source model'),
...
'behavior_change':('adoption','usage','spending','chi tiêu','mua sắm','subscription','subscribers','thói quen','hành vi','consumer','khách hàng','người trẻ','gen z','creator economy','livestream','thanh toán','payment','e-commerce'),
...
def clusters(events,captured):
    ...
    for row in recent:
        ...
        for i,c in enumerate(out):
            ca=set().union(*(set(x.get('anchors') or []) for x in c)); ct=set().union(*(set(x.get('topic_tokens') or []) for x in c)); s=(.75 if ra and ca and ra&ca else 0)+sim(rt,ct)
            if s>score:best,score=i,s
        if best is not None and score>=.42:out[best].append(row)
        else:out.append([row])
    return out
...
def candidate(cluster,misses):
    ...
    score=min(100,22+min(20,len(cluster)*4)+min(18,max(0,len(sources)-1)*8)+min(18,len(ss)*6)+min(12,mag*4)+(8 if vn else 0)+(10 if mm else 0)); state='investigate' if score>=72 and (len(sources)>=2 or mm or mag>=2) else 'watch' if score>=52 else 'background'; lab=label(cluster); cid=hashlib.sha1((lab.casefold()+'|'+'|'.join(ss)).encode()).hexdigest()[:16]
